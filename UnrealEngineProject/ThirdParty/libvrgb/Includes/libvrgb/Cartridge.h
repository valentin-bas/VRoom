...
namespace vrgb
{
	...
	private:
		...
		void CheckCartridge(const char* batteriesPath = "");
		int CheckRomSize(int numHeaderSize, int fileSize);
	public:
		Cartridge(const char* fileName, const char* batteriesPath = "");
		Cartridge(BYTE * cartridgeBuffer, unsigned long size, const char* batteriesPath = "");
		~Cartridge();
