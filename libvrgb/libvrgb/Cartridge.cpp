 ...
 */
Cartridge::Cartridge(const char* fileName, const char* batteriesPath)
{
	_memCartridge = NULL;
 ...
 */
Cartridge::Cartridge(BYTE * cartridgeBuffer, unsigned long size, const char* batteriesPath)
{
	_romSize = size;
 ...
 */
void Cartridge::CheckCartridge(const char* batteriesPath)
{
	...
	for (int i = 0; i < 16; ++i)
		_name[i] = _memCartridge[CART_NAME + i];
	_name[16] = '\0';
